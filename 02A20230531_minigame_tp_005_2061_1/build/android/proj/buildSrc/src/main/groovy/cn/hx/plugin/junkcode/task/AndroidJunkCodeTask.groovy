...
class AndroidJunkCodeTask extends DefaultTask {
                ...
                if (config.typeGenerator) {
                    ...
                    for (int k = 0; k < config.methodCountPerClass; k++) {
                        ...
                        if (config.methodGenerator) {
                            ...
                            generateMethods(methodBuilder, packageName)
                        }
    ...
    static void generateMethods(MethodSpec.Builder methodBuilder, String packageName) {
        def name = JunkUtil.getName(packageName)
        switch (random.nextInt(5)) {
            case 0:
                methodBuilder.addStatement("long now = \$T.currentTimeMillis()", System.class)
                        .beginControlFlow("if (\$T.currentTimeMillis() < now)", System.class)
                        .addStatement("\$T.out.println(\$S)", System.class, name + " travelling!")
                        .nextControlFlow("else if (\$T.currentTimeMillis() == now)", System.class)
                        .addStatement("\$T.out.println(\$S)", System.class, name + " stood still!")
                        .nextControlFlow("else")
                        .addStatement("\$T.out.println(\$S)", System.class, name + " still moving forward")
                        .endControlFlow()
                break
            case 1:
            ...
            case 2:
                methodBuilder.beginControlFlow("try")
                        .addStatement("throw new Exception(\$S)", name + " now Exception")
            ...
            case 4:
                methodBuilder.addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                        ...
                        .addStatement("\$T.out.println(\$S)", System.class, name + " what?")
                break
            default:
                methodBuilder.addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                        .returns(void.class)
                        .addParameter(String[].class, "args")
                        .addStatement("\$T.out.println(\$S)", System.class, "I love " + name)
        }
    }
